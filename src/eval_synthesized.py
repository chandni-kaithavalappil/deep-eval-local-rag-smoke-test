"""Run DeepEval over LightRAG's answers to the synthesized Q&A set.

Same as eval_handcrafted.py minus the RefusalCorrectness metric: synthesized
questions all have answers in the corpus, so refusal isn't relevant here.
"""
import json
import asyncio
import os
...
QA_PATH = Path(os.getenv("QA_PATH", "tests/synthesized_qa.json"))
RESULTS_PATH = Path(os.getenv("RESULTS_PATH", "results/synthesized_scores.json"))
...
DEEPEVAL_METRICS = os.getenv(
    "DEEPEVAL_METRICS",
    "answer_relevancy,faithfulness,contextual_relevancy,contextual_recall",
)
...
def main() -> None:
    ...
    judge_model = None
    if any(metric != "exact_match" for metric in selected_metrics):
